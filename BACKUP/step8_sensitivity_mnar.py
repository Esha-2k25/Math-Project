...
# step8_sensitivity_fixed.py — Sensitivity for Fixed MCDD
import numpy as np
import os
...
def mcdd_fixed(M_obs, lam2=None, rho=1.0, max_iter=300, tol=1e-4,
    ...
    m, n = M_obs.shape
    if lam2 is None: lam2 = 1.0 / np.sqrt(max(m, n))
    ...
    L = M_fill.copy(); S_tamper = np.zeros((m,n)); Y = np.zeros((m,n))
    for i in range(max_iter):
        L_old = L.copy()
        L, rank = svd_threshold(M_fill - S_tamper + Y/rho, 1.0/rho)
        S_new = soft_threshold(M_fill - L + Y/rho, lam2/rho)
        S_new[~mask_obs] = 0; S_tamper = S_new
        res = M_fill - L - S_tamper; res[~mask_obs] = 0; Y += rho * res
        if np.linalg.norm(L-L_old,'fro')/(np.linalg.norm(L_old,'fro')+1e-10) < tol: break
    ...
    if len(crash_cols) > 0 and len(safe_cols) >= 2:
        ...
        U_k = U_safe[:, :k]
        for c in crash_cols:
            obs_rows = mask_obs[:, c]; missing_rows = ~obs_rows
            if obs_rows.sum() >= k:
                A = U_k[obs_rows, :]; b = L[obs_rows, c]
    ...
    for it in range(mnar_iter):
        ...
        mnar_resid = sig - miss_mask; sig_deriv = sig * (1.0 - sig)
...
print("="*65)
print("SWEEP: alpha (sigmoid steepness) | lam2 default, rho=1.0")
print("="*65)
print(f"{'alpha':<10} {'RMSE':<10} {'Theta sep':<12} {'Crashed cols'}")
print("-"*65)

for alpha in [0.5, 1.0, 1.5, 2.0, 3.0]:
    L, theta = mcdd_fixed(M_B, alpha=alpha)
    ...
    print(f"{alpha:<10.1f} {r:<10.4f} {sep:<12.4f} {len(crash_cols)}")
